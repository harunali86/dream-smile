...
async function main() {
    console.log('==================================================');
    console.log('📊 CREATING SIDE-BY-SIDE VISUAL COMPARISON');
    console.log('==================================================\n');

    const inputPath = path.join(BASE_DIR, 'public/demo-result.jpg');
    const resultPath = path.join(BASE_DIR, 'public/test-no-mask-result.webp');
    const sideBySidePath = path.join(BASE_DIR, 'public/comparison-side-by-side.png');
    ...
    try {
        ...
        fs.copyFileSync(inputPath, path.join(ARTIFACTS_DIR, 'experiment-original.jpg'));
        fs.copyFileSync(resultPath, path.join(ARTIFACTS_DIR, 'experiment-no-mask-result.webp'));
        fs.copyFileSync(sideBySidePath, path.join(ARTIFACTS_DIR, 'experiment-side-by-side.png'));
