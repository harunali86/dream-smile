...
const ARTIFACT_PATH = '/home/harun/.gemini/antigravity/brain/99d6af72-1fb8-4c07-b586-f5ba0d6acf54/artifacts/test-composed.webp';
...
async function run() {
    ...
    console.log('3. Blending and composing back onto original image...');
    const cropMeta = roiInput.meta;
    const roi = fs.readFileSync(ROI_OUTPUT_PATH); 
    ...
    const maskedRoi = await sharp(maskedRaw, {
        raw: { width: cropMeta.width, height: cropMeta.height, channels: 4 },
    })
        .png()
        .toBuffer();

    const composed = await sharp(inputBuf)
        .ensureAlpha()
        .composite([
            {
                input: maskedRoi,
    ...
    fs.writeFileSync(ARTIFACT_PATH, composed);
    console.log(`Successfully generated and saved visual blend to: ${ARTIFACT_PATH}`);
}
